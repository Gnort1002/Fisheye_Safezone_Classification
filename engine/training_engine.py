...
from .utils import autocast_fn, get_batch_size, get_log_writers, log_metrics
from utils.tensor_utils import reduce_tensor_sum
from options.parse_args import parse_validation_metric_names

class Trainer(object):
    """
    This class defines the training and validation code for training modes
    """
    ...
    def __init__(
        ...
        self.accum_after_epoch = getattr(
            self.opts, "common.accum_after_epoch", 0)
        self.mixed_precision_training = getattr(
            opts, "common.mixed_precision", False)
        ...
        self.log_writers = get_log_writers(
            self.opts,
            save_location=self.save_location)

        # recent versions of PyTorch support
        # setting grads to None, for better performance
        # To be explored in Future
        # self.optimizer.zero_grad(set_to_none=True)
        self.set_grad_to_none = False

        save_interval_freq = getattr(opts, "common.save_interval_freq", 0)
        # save interval checkpoints every
        # `save_interval_freq` updates on the master node
        self.save_interval = save_interval_freq > 0
