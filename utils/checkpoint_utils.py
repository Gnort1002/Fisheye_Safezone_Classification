...
from .common_utils import unwrap_model_fn
from .ddp_utils import is_master
from .download_utils import get_local_path
...
def avg_and_save_k_checkpoints(
    ...
    """
    try:
        ckpt_fname = "{}_score_{:.4f}.{}".format(
            ckpt_str, best_metric, CHECKPOINT_EXTN
            )
        ...
        if len(best_scores) > k_best_checkpoints:
            ...
            for k in best_scores:
                if k in best_scores_keep:
                    continue
                rm_ckpt = "{}_score_{:.4f}.{}".format(
                    ckpt_str, k, CHECKPOINT_EXTN)
                os.remove(rm_ckpt)
